...
public class String03P399 {
	...
	public static void main(String[] args) {
		...
		String serchVoca = scan.nextLine();
		...
		boolean bool = true;
		//마지막으로 발견된 지점
		int fidx = -1;  // fidx = finalIndex.
		while(bool) {
			// fidx가 마지막으로 조회된 지점이므로
			// fidx +1부터 단어를 찾기 시작해야함.
			// indexOf를 실행하는데 -1이 나온다면
			// 더 찾을 필요가 없다는 의미임.
			int get = sentence.indexOf(serchVoca, fidx + 1);
			if(get == -1) {
				...
				System.out.println(get + "번에서 " + count + "번째 " + 
					               serchVoca + "이(가) 검출되었습니다.");
			}
			System.out.println(serchVoca + "의 총 출현 횟수는 " + count + " 입니다.");
		}
	}
}